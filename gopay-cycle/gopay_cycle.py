...
用法：
  # 单步执行
  python3 gopay_cycle.py --step change-phone --new-phone <new-phone>
  python3 gopay_cycle.py --step deactivate
  python3 gopay_cycle.py --step login --phone <phone>
  python3 gopay_cycle.py --step unlink
  python3 gopay_cycle.py --step status

  # 完整步骤（接入支付流程）
  python3 gopay_cycle.py --step full-cycle --phone <phone> --pin <pin>

环境变量：
  HEROSMS_API_KEY  - HeroSMS 接码平台 API key
...
def get_sms_client() -> HeroSMS:
    if not HEROSMS_KEY:
        return None
    return HeroSMS(HEROSMS_KEY)


def wait_otp(prompt: str = "Enter OTP: ", activation_id: str = None) -> str:
    """等待 OTP：优先用 HeroSMS 自动接收，否则手动输入"""
    sms = get_sms_client()
    if sms and activation_id:
        print("  Waiting for OTP via HeroSMS...")
        code = sms.wait_for_code(activation_id, timeout=120)
        print("  → OTP received")
        return code
...
def change_phone(state, new_phone: str, pin: str):
    """改手机号：3步。new_phone 为空时自动从 HeroSMS 获取"""
    c = get_client(state)
    ...
    # 如果没指定号码，从 HeroSMS 获取临时号
    if not new_phone and sms:
        print("  Getting temp number from HeroSMS...")
        activation_id, new_phone = sms.get_number(service=HEROSMS_SERVICE, country=6, max_price=0.1)
        # 去掉国家码前缀
        if new_phone.startswith("62"):
            new_phone = new_phone[2:]
        print(f"  → Got number: +62{new_phone}")
        state["herosms_activation_id"] = activation_id
        save_state(state)
    ...
    if r["status"] == 461:
        ...
        if sms and activation_id:
            sms.cancel_activation(activation_id)
    ...
    if r["status"] == 200:
        ...
        if sms and activation_id:
            sms.set_status(activation_id, 1)  # 通知 SMS 已发送
    elif r["status"] == 429:
        if sms and activation_id:
            sms.cancel_activation(activation_id)
        raise RuntimeError(f"Rate limited (429): {r['data']}")
    ...
    # Step 3: 等待 OTP
    otp = wait_otp("Enter OTP received on new phone: ", activation_id)
    otp_token = r["data"].get("otp_token", "")
    ...
    if r["status"] == 200:
        if GOPAY_CHANGE_PHONE_COUNTRY_SYNC:
            ...
            if sync["status"] != 200:
                if sms and activation_id:
                    sms.finish_activation(activation_id)
        ...
        if sms and activation_id:
            sms.request_additional_sms(activation_id)  # 保持号码等待注销 OTP
        save_state(state)
    else:
        if sms and activation_id:
            sms.finish_activation(activation_id)
...
def deactivate(state, pin: str):
    ...
    c = get_client(state)
    sms = get_sms_client()
    activation_id = state.get("herosms_activation_id")
    ...
    otp = wait_otp("Enter OTP received on phone: ", activation_id)
...
def unlink(state):
    ...
    print("Getting linked apps...")
    r = c.get(f"{GOPAY_CUSTOMER}/v1/linkedapps")
    if r["status"] != 200:
        print(f"  → Failed: {r}")
        return

    services = r["data"].get("linked_services", [])
    if not services:
        print("  → No linked services")
        return

    for svc in services:
        name = svc.get("service_name", "")
        link_url = svc.get("unlink_service_url", "")
        if link_url:
            print(f"  Unlinking {name}...")
            r2 = c.patch(f"{GOPAY_CUSTOMER}{link_url}")
            print(f"    → {r2['status']}")
...
def full_cycle(state, main_phone: str, pin: str, session_token: str = None):
    """
    完整循环：改号 → 注销 → 登录 → 支付 → 解绑
    """
    print("=" * 60)
    print("GoPay Full Cycle")
    print("=" * 60)

    # Step 1: 改手机号到临时号
    print("\n[STEP 1] Change phone to temp number...")
    change_phone(state, "", pin)  # 空号码 = 从 HeroSMS 获取
    ...
    print("\n" + "=" * 60)
    print("Cycle complete! Ready for next round.")
    print("=" * 60)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--step", required=True, choices=["change-phone", "deactivate", "login", "unlink", "status", "full-cycle"])
    parser.add_argument("--new-phone", help="新手机号 (不含国家码，空则从HeroSMS获取)")
    parser.add_argument("--phone", help="主号码 (不含国家码)")
    parser.add_argument("--pin", default=GOPAY_PIN, help="GoPay PIN (默认读 GOPAY_PIN 环境变量)")
    parser.add_argument("--session-token", help="ChatGPT session token (full-cycle用)")
    ...
    if args.step == "change-phone":
        ...
        if not args.phone:
            print("需要 --phone (主号码)")
            sys.exit(1)
        full_cycle(state, args.phone, args.pin, args.session_token)
